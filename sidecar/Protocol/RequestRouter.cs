...
public class RequestRouter
{
    ...
    public RequestRouter(ServiceBusAdminService admin, ServiceBusMessagingService messaging)
    {
        _handlers = new Dictionary<string, Func<JsonElement?, Task<object>>>
        {
            ...
            ["createQueue"] = p => admin.CreateQueueAsync(p),
            ["createTopic"] = p => admin.CreateTopicAsync(p),
            ["createSubscription"] = p => admin.CreateSubscriptionAsync(p),
            ["deleteQueue"] = p => admin.DeleteQueueAsync(p),
            ["deleteTopic"] = p => admin.DeleteTopicAsync(p),
            ["deleteSubscription"] = p => admin.DeleteSubscriptionAsync(p),
